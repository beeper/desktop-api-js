...
export class BaseSAS extends APIResource {
  ...
  confirm(verificationID: string, options?: RequestOptions): APIPromise<SASConfirmResponse> {
    return this._client.post(path`/v1/app/verifications/${verificationID}/sas/confirm`, options);
  }
  ...
  start(verificationID: string, options?: RequestOptions): APIPromise<SASStartResponse> {
    return this._client.post(path`/v1/app/verifications/${verificationID}/sas/start`, options);
  }
}
/**
 * First-party sign-in and encrypted messaging setup for Beeper Desktop.
