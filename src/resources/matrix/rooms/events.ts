...
export class BaseEvents extends APIResource {
  static override readonly _key: readonly ['matrix', 'rooms', 'events'] = Object.freeze([
    'matrix',
    'rooms',
    'events',
  ] as const);
   ...
   * @example
   * ```ts
   * const event = await client.matrix.rooms.events.retrieve(
  ...
  retrieve(
    ...
    const { roomId } = params;
    return this._client.get(path`/_matrix/client/v3/rooms/${roomId}/event/${eventID}`, options);
  }
}
...
export interface EventRetrieveResponse {
  ...
  unsigned?: EventRetrieveResponse.Unsigned;
}
