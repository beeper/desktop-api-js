...
describe('resource messages', () => {
  test('update: only required params', async () => {
    const responsePromise = client.messages.update('messageID', { chatID: '!NCdzlIaMjZUmvmvyHU:beeper.com', text: 'x' });
  ...
  test('update: required and optional params', async () => {
    const response = await client.messages.update('messageID', { chatID: '!NCdzlIaMjZUmvmvyHU:beeper.com', text: 'x' });
  });
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.messages.list('!NCdzlIaMjZUmvmvyHU:beeper.com', { cursor: '1725489123456|c29tZUltc2dQYWdl', direction: 'before' }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('search: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.messages.search({
  ...
  test('send: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.messages.send('!NCdzlIaMjZUmvmvyHU:beeper.com', {
    attachment: {
    uploadID: 'uploadID',
    duration: 0,
    fileName: 'fileName',
