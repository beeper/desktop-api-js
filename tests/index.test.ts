...
describe('logging', () => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new BeeperDesktop({ logger: logger, accessToken: 'My Access Token' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'BEEPER_DESKTOP_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new BeeperDesktop({ baseURL: 'http://localhost:5000/custom/path/', accessToken: 'My Access Token' });
    ...
    test('in request options overridden by client options', () => {
      const client = new BeeperDesktop({ accessToken: 'My Access Token', baseURL: 'http://localhost:5000/client' });
