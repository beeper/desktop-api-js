...
export const searchChatsHandler: CustomHandlerFunction = async (client, args) => {
  const output = await client.chats.search(args);
  ...
  if (items.length === 0) {
    ...
    for (const chat of items) {
      lines.push(...formatChatToMarkdown(chat, client.baseURL));
    }
  }
