...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { BEEPER_ACCESS_TOKEN: requireValue(readEnv('BEEPER_ACCESS_TOKEN') ?? client.accessToken, 'set BEEPER_ACCESS_TOKEN environment variable or provide accessToken client option'), BEEPER_DESKTOP_BASE_URL: readEnv('BEEPER_DESKTOP_BASE_URL') ?? client.baseURL ?? undefined };
